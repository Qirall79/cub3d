...
/*   Updated: 2024/05/18 11:00:13 by wbelfatm         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	draw_wall(t_config *config, t_vector p, float alpha, int x)
{
	...
	if (p.z)
		texture_pos.x = ((int)p.y % TEX_WIDTH);
	else
		texture_pos.x = ((int)p.x % TEX_WIDTH);
	draw_ceiling_floor(config, start_y, end_y, x);
	y = start_y;
	while (y < end_y && y < HEIGHT)
	{
		texture_pos.y = (int)((y - start_y)
				* ((float) TEX_HEIGHT / wall_height));
		if (y < 0)
			y = 0;
