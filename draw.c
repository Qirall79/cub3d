...
/*   Updated: 2024/04/24 12:05:27 by wbelfatm         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	while (i < MAP_HEIGHT)
	{
		...
		while (j < MAP_WIDTH)
		{
			...
			while (x < (i + 1) * div)
			{
				...
				while (y < (j + 1) * div)
				{
					if (in_range(x, config->xPos * div + config->xOffset, div * (config->xPos + 1) + config->xOffset - 1)
					&& in_range(y, config->yPos * div + config->yOffset, div * (config->yPos + 1) + config->yOffset - 1))
